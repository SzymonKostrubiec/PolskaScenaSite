...
import { buildEnumObject, getResourcePaths, getType, removeTrailingSlash, } from "../core/utils/index.js";
/**
 ...
 */
function assignResourceRelationships(resources) {
...
function mergeResources(resourceA, resourceB) {
...
function getArrayType(property) {
    if (property.type !== "array") {
        return null;
    }
    return getType(property.items.type || "string", property.items.format);
}
function buildResourceFromSchema(schema, name, title, url) {
...
function buildOperationFromPathItem(httpMethod, operationType, pathItem) {
    return new Operation(pathItem.summary || operationType, operationType, {
        method: httpMethod.toUpperCase(),
        deprecated: !!pathItem.deprecated,
    });
}
function dereferenceOpenAPIV3(response, options) {
...
*/
export default async function handleJson(response, entrypointUrl) {
    ...
    for (const path of paths) {
        ...
        const { get: showOperation, put: putOperation, patch: patchOperation, delete: deleteOperation, } = pathItem;
        const editOperation = putOperation || patchOperation;
        if (!showOperation && !editOperation) {
            continue;
        }
        const showSchema = showOperation?.responses?.["200"]?.content?.["application/json"]
            ?.schema || document.components?.schemas?.[title];
        const editSchema = editOperation?.requestBody?.content?.["application/json"]?.schema || null;
        ...
        if (listOperation?.parameters) {
            resource.parameters = listOperation.parameters.map((parameter) => new Parameter(parameter.name, parameter.schema?.type ? getType(parameter.schema.type) : null, parameter.required || false, parameter.description || "", parameter.deprecated));
        }
