 ...
 */
function getTitleOrLabel(obj) {
    const a = obj["http://www.w3.org/2000/01/rdf-schema#label"] ??
...
export function getDocumentationUrlFromHeaders(headers) {
    ...
    if (typeof matches[1] !== "string") {
        throw new TypeError('The "Link" HTTP header does not contain a documentation URL.');
    }
    return matches[1];
}
/**
 ...
 */
async function fetchEntrypointAndDocs(entrypointUrl, options = {}) {
    /**
     ...
     */
    async function documentLoader(input) {
...
function findRelatedClass(docs, property) {
    ...
    for (const range of property["http://www.w3.org/2000/01/rdf-schema#range"] ??
        []) {
        const equivalentClass = "http://www.w3.org/2002/07/owl#equivalentClass" in range
            ? range?.["http://www.w3.org/2002/07/owl#equivalentClass"]?.[0]
            : undefined;
        if (!equivalentClass) {
            continue;
        }
        const onProperty = equivalentClass["http://www.w3.org/2002/07/owl#onProperty"]?.[0]?.["@id"];
 ...
 */
export default async function parseHydraDocumentation(entrypointUrl, options = {}) {
    entrypointUrl = removeTrailingSlash(entrypointUrl);
    const { entrypoint, docs, response } = await fetchEntrypointAndDocs(entrypointUrl, options);
    const resources = [], fields = [], operations = [];
    const title = docs?.[0]?.["http://www.w3.org/ns/hydra/core#title"]?.[0]?.["@value"] ??
        "API Platform";
    ...
    for (const properties of entrypointClass["http://www.w3.org/ns/hydra/core#supportedProperty"]) {
        ...
        const url = typeof resourceProperty === "object"
            ? resourceProperty["@id"]
            : undefined;
        if (!url) {
            console.error(new Error(`Unable to find the URL for "${propertyIri}" in the entrypoint, make sure your API resource has at least one GET collection operation declared.`));
            continue;
        }
        // Add fields
        ...
        for (const supportedProperties of relatedClass?.["http://www.w3.org/ns/hydra/core#supportedProperty"] ?? []) {
            ...
            const range = supportedProperty?.["http://www.w3.org/2000/01/rdf-schema#range"]?.[0]?.["@id"] ?? null;
            const field = new Field(supportedProperties?.["http://www.w3.org/ns/hydra/core#title"]?.[0]?.["@value"] ??
                ...
                embedded: supportedProperty?.["@type"]?.[0]?.endsWith("#Link")
                    ? null
                    : range, // Will be updated in a subsequent pass
                required: supportedProperties?.["http://www.w3.org/ns/hydra/core#required"]?.[0]?.["@value"] ?? false,
                description: supportedProperties?.["http://www.w3.org/ns/hydra/core#description"]?.[0]?.["@value"] ?? "",
                maxCardinality: supportedProperty?.["http://www.w3.org/2002/07/owl#maxCardinality"]?.[0]?.["@value"] ?? null,
                deprecated: supportedProperties?.["http://www.w3.org/2002/07/owl#deprecated"]?.[0]?.["@value"] ?? false,
            });
            fields.push(field);
            ...
            if (supportedProperties?.["http://www.w3.org/ns/hydra/core#writeable"]?.[0]?.["@value"] ||
                supportedProperties?.["http://www.w3.org/ns/hydra/core#writable"]?.[0]?.["@value"]) {
        ...
        if (property["http://www.w3.org/ns/hydra/core#supportedOperation"]) {
            for (const entrypointOperation of property["http://www.w3.org/ns/hydra/core#supportedOperation"]) {
                ...
                const operation = new Operation(getTitleOrLabel(entrypointOperation), type, {
                    method,
                    expects: entrypointOperation["http://www.w3.org/ns/hydra/core#expects"]?.[0]?.["@id"],
                    returns: range,
                    types: entrypointOperation["@type"],
                    deprecated: entrypointOperation?.["http://www.w3.org/2002/07/owl#deprecated"]?.[0]?.["@value"] ?? false,
                });
        ...
        for (const supportedOperation of relatedClass["http://www.w3.org/ns/hydra/core#supportedOperation"] || []) {
            ...
            const operation = new Operation(getTitleOrLabel(supportedOperation), type, {
                method,
                expects: supportedOperation["http://www.w3.org/ns/hydra/core#expects"]?.[0]?.["@id"],
                returns: range,
                types: supportedOperation["@type"],
                deprecated: supportedOperation?.["http://www.w3.org/2002/07/owl#deprecated"]?.[0]?.["@value"] ?? false,
            });
            resourceOperations.push(operation);
            operations.push(operation);
        }
        const resource = new Resource(guessNameFromUrl(url, entrypointUrl), url, {
            ...
            deprecated: relatedClass?.["http://www.w3.org/2002/07/owl#deprecated"]?.[0]?.["@value"] ?? false,
        });
        resource.parameters = [];
